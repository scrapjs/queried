 ...
 */
function queryMultiple(selector, el, incSelf) {
	//ignore bad selector
	if (!selector) return [];
	...
	//if element isn’t a node - make it doc
	...
	return qPseudos(el, selector, incSelf);
}


/** Query single element - no way better than return first of multiple selector */
function querySingle(selector, el, incSelf){
 ...
 */
function qPseudos(list, selector, incSelf) {
	...
	if (match) {
		...
		list = qList(list, preSelector, incSelf);
	...
	//just query list
		...
		// console.groupEnd();
		return qList(list, selector, incSelf);
	}
}


/** Apply selector on a list of elements, no polyfilled pseudos */
function qList(list, selector, incSelf){
	var res = incSelf ? list : [];
	return unique(res.concat(arrayify(list.map(function(el){
